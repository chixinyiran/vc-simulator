...
const Sfx = (function(){
  ...
  const lib={
    ...
    // 封存确认：银钱洒落——低频盖章落定 + 一串高频金属额颁错落洒落 + 余响(有气势财富感)
    confirm(){
      // ① 低频盖章咚打底(落定厘额)
      tone(196,0,0.20,'sine',0.6,110);
      tone(98,0.01,0.26,'sine',0.45,70);
      // ② 银钱洒落:一串高频金属额颁,频率随机、时间错落(像钱币撒下)
      var coins=[2637,3136,2349,2794,3520,2093,2960,2489];
      for(var i=0;i<coins.length;i++){
        var t=0.04+i*0.035+Math.random()*0.02;
        var pk=0.30+Math.random()*0.18;
        tone(coins[i], t, 0.16, 'triangle', pk);
        tone(coins[i]*1.5, t+0.005, 0.10, 'sine', pk*0.4);
      }
      // ③ 尾部余韵(高频轻点)
      tone(4186,0.34,0.22,'sine',0.18);
      tone(3520,0.40,0.26,'sine',0.14);
    },
